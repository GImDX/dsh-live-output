...
window.__ModuleLoader__.load({
  ...
  factory: function (require) {
    ...
    function gitLineClassify(line) {
      ...
      if (/^@@(-\d|{)/.test(line) || /^@@@ /.test(line)) return GIT_STYLES.hunk; // 常规/合并 diff
      ...
      // 英文/中文 locale 的 status 行
      if (/^On branch |^Your branch |^\u4f4d\u4e8e\u5206\u652f |^\u60a8\u7684\u5206\u652f /.test(line)) return GIT_STYLES.statusTitle;
      if (/^Changes (not staged|to be committed)/.test(line)) return GIT_STYLES.statusTitle;
      if (/^\u5c1a\u672a\u6682\u5b58\u4ee5\u5907\u63d0\u4ea4\u7684\u53d8\u66f4/.test(line)) return GIT_STYLES.statusTitle;
      if (/^\s*modified:|^\s*\u4fee\u6539\uff1a/.test(line)) return GIT_STYLES.statusModified;
      if (/^\s*new file:|^\s*\u65b0\u6587\u4ef6\uff1a/.test(line)) return GIT_STYLES.statusNew;
      if (/^\s*deleted:|^\s*\u5220\u9664\uff1a/.test(line)) return GIT_STYLES.statusDeleted;
      if (/^nothing to commit|^\u65e0\u6587\u4ef6\u8981\u63d0\u4ea4/.test(line)) return GIT_STYLES.statusClean;
      return null;
    }
    ...
    function renderCodeLine(line) {
      ...
      while (i < n) {
        ...
        if (ch === "#") {
          // 十六进制颜色（#ff0000 / #fff）不是注释
          var afterHash = line.slice(i + 1);
          if (/^[0-9a-fA-F]{3,8}(?![\w])/.test(afterHash)) { i++; continue; }
          flushPlain(i);
          frags.push({ text: line.slice(i), style: CODE_STYLES.comment });
          return frags;
        }
        if (ch === "/" && line[i + 1] === "/" && /^\s*$/.test(line.slice(0, i))) {
          // 仅行首（缩进后）的 // 视为注释，避免 Python 整除 a // b 误伤
          flushPlain(i);
          frags.push({ text: line.slice(i), style: CODE_STYLES.comment });
          return frags;
        }
    ...
    /** 是否启用代码着色：命令感知（行首或命令分隔符后，大小写不敏感）；超长文本退化。 */
    function codeMode(command, text) {
      if (text.length > 32 * 1024) return false;
      if (typeof command !== "string") return false;
      // 命令起点（行首或 ;/&/| 之后）的 cat/type/more/gc/Get-Content 才触发，
      // 避免自然语言（echo one more thing）误触发
      return /(^|[;&|]\s*)(cat|type|more|gc|get-content)(\s|$)/i.test(command);
    }
    ...
    function renderAnsi(text) {
      ...
      var style = null;
      var match;
      var k = 0; // 局部序号：同文本渲染时 key 稳定，React 可复用既有节点
      function push(str, st) {
        if (str === "") return;
        nodes.push(e("span", { key: "a" + (k++), style: st ?? undefined }, str));
      }
      ANSI_RE.lastIndex = 0;
    ...
    function renderPlain(text, gitEnabled, codeEnabled) {
      ...
      var k = 0; // 局部序号：同文本渲染时 key 稳定
      for (var i = 0; i < lines.length; i++) {
        var line = lines[i].replace(/\r$/, "");
        if (i > 0) nodes.push(e("span", { key: "n" + (k++), style: undefined }, "\n"));
        if (line === "") continue;
        if (codeEnabled && !gitEnabled) {
          var frags = renderCodeLine(line);
          for (var f = 0; f < frags.length; f++) {
            if (frags[f].text === "") continue;
            nodes.push(e("span", { key: "n" + (k++), style: frags[f].style ?? undefined }, frags[f].text));
          }
          continue;
        }
        var style = null;
        ...
        nodes.push(e("span", { key: "n" + (k++), style: style ?? undefined }, line));
      }
      return nodes;
    }
    ...
    function LiveOutputPanel(props) {
      ...
      function startResize(ev) {
        ...
        // 拖动开始时输出区是否在底部：高度变化后保持滚动位置——
        // 原在底部 → 贴底跟随；否则 → 顶部锚定（防止变矮时浏览器钳位跳底）。
        var bodyAtBottom0 = bodyEl
        ...
        function apply(h) {
          ...
          if (bodyEl) {
            if (bodyAtBottom0) {
              // 贴底跟随（需读 scrollHeight 同步布局一次）
              bodyEl.scrollTop = bodyEl.scrollHeight;
            } else if (h < prevH) {
              // 仅变矮时才可能被浏览器钳位跳底：读几何并恢复顶部锚定；
              // 变高方向零布局读（scrollTop 不会超界，顶部锚定天然成立）
              var st = bodyEl.scrollTop;
              var max = Math.max(bodyEl.scrollHeight - bodyEl.clientHeight, 0);
              bodyEl.scrollTop = Math.min(st, max);
            }
          }
          prevH = h;
        }
      ...
      // 输出渲染按文本+命令缓存（useMemo 必须在组件顶层无条件调用，不可放
      // isOpen 等条件块内）：拖动/计时器/无关 setState 不再全量重建着色 span 树；
      // span key 已随内容稳定，文本增量更新时 React 可复用既有节点。
      var outText = selProc === null ? "" : (texts[selected] || "");
      var outCmd = selProc === null ? undefined : selProc.command;
      var outputNodes = useMemo(function () { return renderOutput(outText, outCmd); }, [outText, outCmd]);
      ...
      if (isOpen) {
        ...
        if (selProc === null) {
          ...
          body = e("pre", { className: "dsh-live-output-body", ref: bodyRef }, outputNodes);
        }
