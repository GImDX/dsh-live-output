...
window.__ModuleLoader__.load({
  ...
  factory: function (require) {
    ...
    /** 无 ANSI 的普通文本：git 行型（启用时）→ diff 前缀 → 关键词，整行着色。 */
    function renderPlain(text, gitEnabled) {
      var nodes = [];
    ...
    /** 渲染输出文本：含 ANSI 走 ANSI 着色，否则 git/关键词行级高亮；超长只渲染尾部。 */
    function renderOutput(text, command) {
      ...
      if (text.length > RENDER_LIMIT) {
        return [e("span", { key: "cut", style: { color: "#6b7280" } }, "…(\u66f4\u65e9\u8f93\u51fa\u5df2\u7701\u7565)\n")].concat(renderPlain(text.slice(text.length - RENDER_LIMIT), gitEnabled));
      }
      return renderPlain(text, gitEnabled);
    }
